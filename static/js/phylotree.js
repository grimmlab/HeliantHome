    ...
    update(transitions) {
      ...
      if (this.options["brush"]) {
        ...
        var brush_object = d3__namespace
          .brush()
          .on("brush", () => {
            var extent = d3__namespace.event.target.extent(),
      ...
      if (this.options["zoom"]) {
        let zoom = d3__namespace.behavior
          .zoom()
          .scaleExtent([0.1, 10])
          .on("zoom", () => {
            let translate = d3__namespace.event.translate;
            translate[0] += this.offsets[1] + this.options["left-offset"];
            translate[1] += this.pad_height();

            d3__namespace.select("." + css_classes["tree-container"]).attr(
              "transform",
              "translate(" + translate + ")scale(" + d3__namespace.event.scale + ")"
            );
          });
